package com.alibaba.extractor.oracle;
...
import com.alibaba.common.db.sql.SqlTemplates;
...
import com.alibaba.applier.AbstractRecordApplier.TableSqlUnit;
                ...
                for (OracleIncrementRecord record : records) {
                    ...
                    try {
                        ...
                        if ((record.getOpType() == IncrementOpType.I || record.getOpType() == IncrementOpType.U)
                            ...
                            record.setDiscardType(OracleIncrementRecord.DiscardType.DELTE_AFTER_IU);
                        } else if (exist && record.getOpType() == IncrementOpType.D) {
                            // 说明后续肯定有一个insert操作,因为mysql那边使用replace,所以这种情况下也不需要做事
                            record.setDiscardType(OracleIncrementRecord.DiscardType.IU_AFTER_DELETE);
                        } else {
                            record.setDiscardType(OracleIncrementRecord.DiscardType.NONE);
                        }
                        rs.close();
